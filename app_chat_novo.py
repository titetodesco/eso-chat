# -*- coding: utf-8 -*-
"""
app_chat_novo.py — v8 (sem filtro LOCATION — final)

Mudança solicitada: **remover totalmente** do menu lateral o campo
"Filtrar LOCATION (lista separada por ;)" e suas implicações, mantendo
**todas as demais funcionalidades** intactas.

Implicações tratadas:
- Nenhum widget/entrada para LOCATION no sidebar.
- A função `filter_sphera(...)` mantém a assinatura, mas **ignora** o
  parâmetro `locations` (para retrocompatibilidade); o filtro por LOCATION
  não é mais aplicado aqui.
- Nenhuma referência visual a LOCATION na UI da sidebar; a coluna LOCATION
  continua visível nas tabelas (apenas exibição, sem filtro).
- Todo o restante (prompts, uploads, dicionários WS/Precursores/CP,
  thresholds, limpar chat/rascunho/uploads, execução somente ao clicar,
  Description completa, datasets_context.md) permanece inalterado.
"""
...
def filter_sphera(df: pd.DataFrame, locations: List[str], substr: str, years: int) -> pd.DataFrame:
    """Mantida assinatura, porém **ignora** `locations` (sem filtro por Location).
    Aplica janela temporal e filtro por substring de Description.
    """
...
def sphera_similar_to_text(query_text: str, min_sim: float, years: int, topk: int,
    ...
    """Busca sem filtro por Location (lista `locations` é ignorada propositalmente)."""
...
if go_btn:
    ...
    hits = sphera_similar_to_text(
        ...
        df_base=df_sph, E_base=E_sph, substr=substr, locations=[],  # sem filtro por Location
    )
